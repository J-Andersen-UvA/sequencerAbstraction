...
| **Sequencer**       | `sequencerTimeChanged()`                                                                                                                                                                                 | Returns true once when the opened sequence changes, then whenever the Sequencer playhead time differs from the previous call.    |
...
| **Sequencer**       | `GetAllAnimSequencesInCurrentSequence()`                                                                                                                                                                        | Returns all animation sequences in the currently opened sequence as `UAnimSequence*`. |
...
| **ControlRig**      | `GetKeysInSelectionRangeForNames(UMovieSceneTrack* Track, const TArray<FName>& Names, bool bMatchContains, FString& ErrorMessage)`                                                                       | Returns Control Rig channel keys inside the current Sequencer selection range, filtered by channel/control names.               |
| **ControlRig**      | `SelectKeysInSelectionRangeForNames(UMovieSceneTrack* Track, const TArray<FName>& Names, bool bMatchContains, bool bClearExistingSelection, bool bThrobSelection, FString& ErrorMessage)`                | Selects matching Control Rig channel keys in Sequencer so they are visibly highlighted in the timeline.                         |
| **ControlRig**      | `SelectKeysAtFrameForNames(UMovieSceneTrack* Track, int32 DisplayFrame, const TArray<FName>& Names, bool bMatchContains, bool bClearExistingSelection, bool bThrobSelection, FString& ErrorMessage)`      | Selects matching Control Rig channel keys at a specific display-rate frame.                                                     |
| **ControlRig**      | `SelectBoneKeysInSelectionRange(UMovieSceneTrack* Track, const TArray<FName>& BoneNames, bool bClearExistingSelection, bool bThrobSelection, FString& ErrorMessage)`                                      | Selects FK bone-control keys from the Control Rig hierarchy; empty `BoneNames` selects all FK bone controls.                    |
| **ControlRig**      | `SelectBoneKeysAtFrame(UMovieSceneTrack* Track, int32 DisplayFrame, const TArray<FName>& BoneNames, bool bClearExistingSelection, bool bThrobSelection, FString& ErrorMessage)`                            | Selects FK bone-control keys at a specific display-rate frame; empty `BoneNames` selects all FK bone controls.                  |
| **ControlRig**      | `RemoveKeysFromSelectionInSelectionRangeForNames(UMovieSceneTrack* Track, const TArray<FName>& Names, bool bMatchContains, FString& ErrorMessage)`                                                        | Removes matching Control Rig channel keys from the current Sequencer key selection.                                             |
| **ControlRig**      | `GetSelectedNamesInTrack(UMovieSceneTrack* Track, const TArray<FName>& Names, bool bMatchContains, FString& ErrorMessage)`                                                                                | Returns requested names that currently have selected keys on the track, useful for highlighting matching UI buttons.            |
...
| **Bake**            | `BakeBindingToControlRig(ULevelSequence* Sequence, UObject* WorldContextObject, const FGuid& BindingGuid, TSubclassOf<UControlRig> ControlRigClass, bool bReduceKeys, float Tolerance, bool bResetControls, FSequenceOpenResult& Result)` | Bakes the specified skeletal mesh binding onto a Control Rig track using Unreal's native Sequencer bake-to-control-rig path. |
